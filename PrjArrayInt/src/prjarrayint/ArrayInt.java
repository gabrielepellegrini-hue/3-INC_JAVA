...
public class ArrayInt {
    ...
    public void shiftDestraParziale(int valore, int posizione) {
        if (dimL < arr.length) {
    ...
    public void addElemOrdinato(int valore) {
        boolean controllo = false;
        int i = 0;
        while (i < dimL && controllo == false) {
            if (arr[i] > valore) {
                shiftDestraParziale(valore,i);
                controllo = true;
            }
            i++;
        }
        if(controllo == false){        //se il valore è il piu grande di tutti
            shiftDestraParziale(valore,dimL);
        }
    }
